...
| `done` | Verification | Verified `pytest` passes with 24 tests; verified `nom-pair-ci`, `nom-one-electron`, `nom-analyze-run`, `nom-exchange-map`, `nom-convergence-study`, and `nom-fd-baseline` smoke paths. |
...
| `in_progress` | One-electron solver quality | Thin runnable neural Block-Ritz core, quality diagnostics, final normalization, min-step early stopping guard, grid convergence automation, and finite-difference reference comparisons exist. | Add stronger stopping criteria and expanded convergence axes. |
