...
### Early stopping and diagnostics cadence

Training now supports deterministic early stopping using a fixed-grid validation eigsum evaluated every configurable number of steps. This is intended to cut off long low-value tails without changing the underlying Block-Ritz objective.

Heavy overlap-conditioning diagnostics are also computed at a lower configurable cadence during training, which reduces per-step overhead while preserving the main loss and eigvalue traces.
